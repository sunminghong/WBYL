	...
	<div class="mw">
		<div id="profile_header">
			...
			<div class="profile_text">
				...
				<? } else { ?><? echo $totalcountlist['name']==$account['name']?'#我太有才了#':'@'.$totalcountlist['name'].'，#你太有才了#';?>————得到了<? if($totalcountlist['wenquxingcount']>0 ) { ?><?=$totalcountlist['wenquxingcount']?>枚文曲星勋章，<? } ?><? if($totalcountlist['boshicount']>0 ) { ?><?=$totalcountlist['boshicount']?>枚博士勋章，<? } ?><? if($totalcountlist['topcount']>0 ) { ?><?=$totalcountlist['topcount']?>枚牛人勋章，<? } ?><? if($totalcountlist['wincount']>0 ) { ?><?=$totalcountlist['wincount']?>枚达人勋章，<? } ?><?=$totalcountlist['alljifen']?>枚#智慧币#！？
				<? } ?>
				</textarea>
