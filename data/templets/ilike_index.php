...
<script>var op="<?=$op?>"; var wbisload=false; var templateurl='<?=$templatepath?>'; var urlbase='<?=$urlbase?>';<? if($account) { ?>var logined=true; var myuid='<?=$account['uid']?>';<? } else { ?>var logined=false;  var myuid='';<? } ?></script>
<script type="text/javascript" src="<?=$templatepath?>/ilike_images/ilike.js?v=0.2"></script>
</head>
<body>
	<div id="topbg">    
    <div id="topcon">    
	...
	<? if($account) { ?>
		<? if($score) { ?>
		<div id="aboutfaner"><img style="height:45px;"  src="<?=$account['avatar']?>" align="right"/><?=$account['screen_name']?>，你现在得<b id="myscore"><?=$score['score']?></b>分，上传<b id="mypiccount"><?=$score['piccount']?></b>个范儿，<br/>被评价<b id="mybyratecount"><?=$score['byratecount']?></b>次，看了<b id="myratecount"><?=$score['ratecount']?></b>个范儿，共评出 <b id="myratescore"><?=$score['ratescore']?></b>分。</div>
		<? } else { ?>
		<div id="aboutfaner"><img style="height:45px;"  src="<?=$account['avatar']?>" align="right"/><?=$account['screen_name']?>，你现在得<b id="myscore">0</b>分，被评价<b id="mybyratecount">0</b>次，上传<b id="mypiccount">0</b>个范儿，<br/>看了<b id="myratecount">0</b>个范儿，共评出 <b id="myratescore">0</b>分。</div>		
		<? } ?>
	<? } else { ?>
    ...
    <div id="main">
    	<div id="left" class="">
        	...
        	<div class="loginfalse" style=" display:none">
                ...
                <a href="javascript:void(0);" id="loginbtn"><img src="<?=$templatepath?>/ilike_images/sina_login_32.png" /></a>
            </div>
