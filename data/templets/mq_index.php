...
<? foreach((array)$mingrenlist as $top) {?>
						<div style="height:45px; overflow:hidden; margin:5px 5px;">
							...
							<img style="height:45px;" src="<?=$top['avatar']?>"/><img style="height:45px;" src="<?=$urlbase?>images/zhengshu_mq_ico_<?=$top['mqlv']?>.png" class="icoview" params="mq:<?=$top['mqlv']?>:<?=$top['uid']?>:太NB了，#看看你有多聪明#的明星排行榜里的@<?=$top['name']?>拥有<?=$top['followers']?>个粉丝，还获得了#<?=$top['ch']?>证书#！" alt="点击查看证书"/>
						</div>
...
<div style="clear:both;"></div>
		<div class="div_login">
		<? if(is_array($account) ) { ?>
			<a href="?app=mq&op=ready"><img src="<?=$templatepath?>/q_images/btn_test_green.gif" alt="我来测试下"/></a>
...
<? } elseif($op=="ready") { ?>
		<div class="contentFrame">
			<div class="ui-widget" style="text-align:center; color:#696a62;">
				<div>
					<strong>微博<? echo strtoupper($app);?>测试约需5分钟，你准备好了吗？</strong>
				</div>
				<div class="welcomeDiv1">
					1。这是国际流行的测试题，你可以用来了解自己的MQ状况。
				</div>
				<div class="welcomeDiv1">
					2。虽然你可以测试多遍，但第一次测试请你认真对待（你懂的）！
				</div>
			</div>
		</div>
		<div class="div_login">
		...
		<? if(is_array($account) ) { ?>
			<a href="?app=mq&op=ican"><img src="<?=$templatepath?>/q_images/btn_ready.gif" alt="我准备好了，开始" /></a>
		<? } else { ?>
<? global $canLogin;?>
	<? foreach((array)$canLogin as $login) {?>
		<a href="?app=home&act=account&op=tologin&lfrom=<?=$login?>" border="0"><img height="24" src="<?=$templatepath?>/images/btn_login_<?=$login?>.png" alt="用微博帐号登录" /></a> 
...
<div class="contentFrame">
			<div class="ui-widget" style="text-align:center; color:#696a62;">
				<div class="welcomeDiv1" id="top_list1" style="text-align:left;">				
					<b><? echo strtoupper($app);?>数据统计</b>：总登录人数 <b><?=$mqCount['totalUser']?></b>，成功测试人数 <b><?=$mqCount['mqs']?></b>人， 有效测试  <b><?=$mqCount['logs']?></b>次。目前@<?=$mqCount['maxName']?> 以  <b><?=$mqCount['maxMq']?></b>分的惊人成绩 排名第一！希望聪明的你可以创造奇迹超越 TA！ 
<br/><br/>
		<a href="javascript:void();" onclick="javascript:sendStats();"><img src="<?=$templatepath?>/q_images/btn_tweettop.gif" alt="发到微博瞻仰一下"/></a>
					...
					<a href="?app=mq&op=ready"><img src="<?=$templatepath?>/q_images/btn_testagain.gif" alt="再测试一次"/></a>
				</div>
			</div>
		</div>
		<? if($myfriendslist) { ?>
			...
			<div class="ui-widget" style="text-align:center; color:#696a62;">
				...
				<div class="welcomeDiv1" id="top_list1" style="text-align:left;">				
					<? foreach((array)$myfriendslist as $top) {?>
						...
						<div style="line-height:45px;height:45px;float:left;">第<b><?=$top['i']?></b>名，@<?=$top['name']?>，最高<? echo strtoupper($app);?><b><?=$top['mq']?></b>，共测试<b><?=$top['testCount']?></b>次</div>
						<img style="float:left;margin-left:15px;" src="<?=$urlbase?>images/zhengshu_mq_ico_<?=$top['mqlv']?>.png" class="icoview" params="mq:<?=$top['mqlv']?>:<?=$top['uid']?>"/>
						</div>
					<? } ?>
