...
<? if($op=="ready") { ?>
	<form method="post" action="" class="c_form">
				...
				<caption>
				<h2><? echo strtoupper($app);?>测试，智商测试</h2>
				<p>通过国内公认的专业<? echo strtoupper($app);?>评测系统，您可以更加客观全面的了解自己和身边朋友的智商水平，这是beta版</p>
				</caption>
				<tr>
				<th style="width:12em;"><a href="?app=eq&op=ican"><img src="<?=$templatepath?>/q_images/btn_ready.gif" alt="我准备好了，开始" /></a></th>
				<td style="text-align:left;">
				<? echo strtoupper($app);?>测试共计33道测试题，为了方便你操作，所有题型都设计成了单选题。为了保证测试的准确性，请你注意以下方面：
						<ul class="test">
...
<? } elseif($op == 'showscore') { ?>
			<div><p>
					<strong>您本次智商测试得分为<?=$score['noweq']?>分 ，用时 <?=$score['newusetime']?></strong></p>
				</div>
...
<form name="quest" method="post" action="?app=eq&op=cacl" target="_self" id="quest_form">
...
<div id="quest_append" style="margin:30px 0 0 30px;"></div>
	<div class="field" style="margin:20px;padding-left:20px;">
			<p>
							<a href="javascript:void(0);" onclick="document.getElementById('quest_form').submit();" ><img src="<?=$templatepath?>/q_images/btn_showscore_<?=$app?>.gif" alt="查看我的分数"/></a></p>
	</div>
</form>
