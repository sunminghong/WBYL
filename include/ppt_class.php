...
class ppt{
	function login($uidarr){
		...
		if($uid){
			$sql="update ".dbhelper::tname("ppt",'user_lfrom')." set logins=logins+1,".$sqlu." where  lfromuid='$lfromuid' and lfrom='$lfrom'";			
			 
			dbhelper::execute($sql);
		}
		else{ //新用户则要添加相应的记录
			$sql="insert into ".dbhelper::tname("ppt",'user')." (name,regtime,lasttime,logintime,logins) values('".$uidarr['name']."',$timestamp,$timestamp,$timestamp,1)";	
			$uid=dbhelper::execute($sql,1);
			
			$sql="insert into ".dbhelper::tname("ppt",'user_lfrom')." set uid=$uid,lfrom='".$uidarr['lfrom']."',lfromuid=$lfromuid,logins=1,regtime=$timestamp,".$sqlu;
		}
