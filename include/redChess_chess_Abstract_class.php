...
class redChess_chess_Abstract{
	...
	public function check(){
		...
		$sql="select max(lasttime) as last from ".dbhelper::tname("redchess","chess_updatelog")." where lfrom='$lfrom' and lfromuid='$lfromuid' and uid=".$this->uid;
	...
	public function init(){
		...
		$this->preSql="REPLACE ".dbhelper::tname("redchess","chess_".$this->lfrom)." (id,lfrom,name,followers,followings,tweets,score,insertno,lasttime,avatar) VALUES";
		$this->spit="";
	...
	public function filish(){
		...
		$sqllib="delete from ".dbhelper::tname("redchess","userchess")." where uid=".$this->uid." and lfrom='$lfrom' and lfromuid='$lfromuid'";
		dbhelper::execute($sqllib);		
		$sqllib="INSERT INTO ".dbhelper::tname("redchess","userchess")." (uid,lfrom,lfromuid,chessid) select ".$this->uid.",'$lfrom','$lfromuid',id from ".dbhelper::tname("red_chess_".$this->lfrom)." where insertno='".$this->insertno."'";		
		dbhelper::execute($sqllib);
		
		$sqllib="INSERT INTO ".dbhelper::tname("redchess","chess_updatelog")." (uid,lfrom,lfromuid,lasttime) select ".$this->uid.",'$lfrom','$lfromuid',".$this->timestamp."";		
		dbhelper::execute($sqllib);
	}
