...
class redChessLib {
	...
	public function loginUser(){
		...
		$sql="select * from ".dbhelper::tname("redchess","user")." where uid='".$uid."'";
		$rs=dbhelper::getrs($sql);
		if ($row = $rs->next()) {			
			...
			$sql="update ".dbhelper::tname("redchess","user")." set logins=logins+1,".$ss." where uid=".$uid; 
		}
		else
			$sql="insert into ".dbhelper::tname("redchess","user")." set uid=".$uid.",score=0,wins=0,losts=0,regtime=$timestamp,".$ss;

		dbhelper::execute($sql);
		
		//记录在线列表
		$sql="replace into ".dbhelper::tname("redchess","online")." (uid,flag,score,lasttime) values (".$uid.",1,0,$timestamp)";		
		dbhelper::execute($sql);
		return $uidarr;
	}	
}
