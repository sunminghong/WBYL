...
//print_r($_FILES);
		...
		if (is_uploaded_file($_FILES[$fieldName]['tmp_name'])) {
			...
			if (move_uploaded_file($_FILES[$fieldName]['tmp_name'], $file_name)) {
				$result = GetImageSize($this->file_name);
	...
	function get_storage_file_name($name, $suffix)
	{
		...
		if(ISSAE){
			$this->file_name =tempnam(SAE_TMP_PATH, "SAE_IMAGE");
		}
		else {
			$this->file_name =DATADIR."photo/".date('YmdHis',gettimestamp())."_".$this->uid."_".rand().".".$suffix;
		}
		return $this->file_name;
	}
