...
//echo $top;
	...
	static function makeIQ($account,$iqscore,$newmake=false) {
		...
		$type="iq";
		$url="";
		if(1==0 && !$newmake && ($url=self::checkZhengshu($uid,$type,$iqlv))){
		...
		if(ISSAE){
			...
			$newName=ROOT."data/zhengshu/{$date}-iq-{$zid}_{$type}_{$uid}_{$iqlv}.png";
		...
		if(ISSAE) {
			...
			$url=self::getBaseUrl($zid)."data/zhengshu/{$date}-iq-{$zid}_{$type}_{$uid}_{$iqlv}.png";
		}
...
//echo $top;
	...
	static function makeEQ($account,$eqscore,$newmake=false) { 
		...
		$type="eq";
		$url="";
		if(1==0 && !$newmake && ($url=self::checkZhengshu($uid,$type,$eqlv))){
			return array("isold"=>1,"zsurl"=>$url);
		}
		...
		if(ISSAE){
			...
			$newName=ROOT."data/zhengshu/{$date}-eq-{$zid}_{$type}_{$uid}_{$eqlv}.png";
